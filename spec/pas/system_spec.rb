...
RSpec.describe PAS::System do
  ...
  context "System, given candidate" do
    before(:each) do
      ...
      # Defined a default value for output.main_stress?, but it needs to be 
      # overridden in contexts where there is no main stress ([s] and [ss]).
  ...
  context "with lexicon including r1 /s./ and s1 /S:/" do
    before(:each) do
      ...
      @gram = instance_double("PAS::Grammar")
