...
RSpec.describe LoserSelectorExhaustive do
  ...
  before(:example) do
    ...
    allow(uninformative).to receive(:ident_viols?).with(winner) \
                                                  .and_return(false)
    @selector =
      LoserSelectorExhaustive.new(sys, erc_list_class: erc_list_class,
  ...
  context 'given an empty ERC list' do
    ...
    msg1 = <<-'MSG1'
      and a competition where the first candidate has identical violations
    MSG1
    context msg1 do
      before(:example) do
    ...
    msg3 = <<-'MSG3'
      and a competition where an uniformative candidate precedes
      an informative one
    MSG3
    context msg3 do
      before(:example) do
  ...
  context 'given an ERC list with one external erc' do
    ...
    msg4 = <<-'MSG4'
      and a competition ordered uninformative, winner, cand1, ident_viols
    MSG4
    context msg4 do
      before(:example) do
      ...
      it 'adds the external erc to erc_list' do
        expect(erc_list).to have_received(:add_all).with([extern_erc]) \
                                                   .at_least(1).times
      end
    end
  end
