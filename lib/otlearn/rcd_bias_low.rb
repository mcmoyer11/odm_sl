...
module OTLearn
  ...
  class RcdBiasLow < Rcd
    ...
    def choose_cons_to_rank(rankable)
      ...
      low_rankable, high_rankable =
        rankable.partition { |con| low_constraint_type?(con) }
      ...
      unless low_active.empty?
        ...
        if best_pair[1]==0
          chosen_list = low_active
        else
          chosen_list = [best_pair[0]]
        end
        return chosen_list
      end
